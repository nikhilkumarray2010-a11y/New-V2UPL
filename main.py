...
BUTTONSCONTACT = InlineKeyboardMarkup([[InlineKeyboardButton(text="☎️ Contact", url="https://t.me/Studentzzzbot")]])
keyboard = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(text="📢 Help", url="https://t.me/Studentzzzbot")        ],
    ]
)
...
async def send_logs(client: Client, m: Message):  # Correct parameter name
    ...
    if m.chat.type == "channel":
        ...
        if not db.is_user_authorized(m.from_user.id, bot_username):
            await m.reply_text("❌ You do not have right to use this command.")
            return
...
async def txt_handler(bot: Client, m: Message):  
    ...
    await editable.edit(f"**1. Send Your Name For Caption ✍️ Credit\n2. Send /d For default Credit **")
    try:
    ...
    chat_id = editable.chat.id
    timeout_duration = 3 if auto_flags.get(chat_id) else 20
    await editable.edit(f"**1. Send PW Token 🎞️ For MPD urls\n 2. Send /d For Others **")
    try:
    ...
    try:
        if raw_text == "1":
            batch_message = await bot.send_message(chat_id=channel_id, text=f"<blockquote><b>📚 𝐂ⱺᴜʀꜱᴇ NAME : {b_name}</b></blockquote>")
            if "/d" not in raw_text7:
                await bot.send_message(chat_id=m.chat.id, text=f"<blockquote><b><i>📚 𝐂ⱺᴜʀꜱᴇ NAME : {b_name}</i></b></blockquote>\n\n🔄 Your Task is under processing, please check your Set Channel📱. Once your task is complete, I will inform you 📩")
                await bot.send_message(chat_id=m.chat.id, text=f"<blockquote><b><i>📚 𝐂ⱺᴜʀꜱᴇ NAME : {b_name}</i></b></blockquote>\n\n🔄 Your Task is under processing, please check your Set Channel📱. Once your task is complete, I will inform you 📩")
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            raw_line = links[i][0]   # <-- Yeh line add karo
            ...
            elif "static-db-v2.classx.co.in/" in url:
                if "*" in url:
                    base_url, key = url.split("*", 1)
                    base_url = base_url.split("?")[0]
            ...
            if any(x in url for x in ["https://cpvod.testbook.com/", "classplusapp.com/drm/", "media-cdn.classplusapp.com", "media-cdn-alisg.classplusapp.com", "media-cdn-a.classplusapp.com", "tencdn.classplusapp", "videos.classplusapp", "webvideos.classplusapp.com"]):
                ...
                api_url_call = f"https://shefu-api-final.vercel.app/shefu?url={url}@ITSGOLU_FORCE&user_id=7445620075"
                keys_string = ""
                mpd = None
                try:
                    ...
                    # Non-DRM response (direct url)
                        ...
                        await m.reply_text("⚠️ returned unexpected response, attempting fallback...")
            ...
            # keep some older checks for coverage
            ...
            elif "video-streaming-source.s3.ap-south-1.amazonaws.com" in url:
                print(f"🔄 Adda247 S3 video detect: {url}")
                api_base = "https://nova-baba-code.vercel.app/adda"
                token= "eyJhbGciOiJIUzI1NiJ9..n7XwSnoKx51oBTWa9qqBfcXUiqX9sjpKasvuHnY-0VM"
                quality = raw_text2  # 720 / 480 etc user input
                ...
                Show = (
                    f"**⚡ Adda247 S3 Download Started...⏳**\n"
                    f"🔗 URL: `{url}`\n"
                    f"✦ Bot Made By ✦ {CREDIT} "
                )
                ...
                try:
                    ...
                    print(f"❌ Adda247 S3 Error: {str(e)}")
                       await bot.send_message(
                           channel_id,
                           f"⚠️ **Adda247 S3 Download Failed** ⚠️\n"
            ...
            elif "store.adda247.com" in url and (url.endswith(".doc") or url.endswith(".pdf")):
                print(f"🔄 Adda247 Store file detect: {url}")
                JWT_TOKEN = "eyJhbGciOiJIUzI1NiJ9..n7XwSnoKx51oBTWa9qqBfcXUiqX9sjpKasvuHnY-0VM"  # <-- YAHAN CHANGE KARO
                headers = {
                    "Authorization": f"Bearer {JWT_TOKEN}",
                    "User-Agent": "Mozilla/5.0 (Linux; Android 13; SM-G998B) AppleWebKit/537.36",
                    "Referer": "https://store.adda247.com/",
                    "Accept": "*/*"
                }
                filename = name.split("/")[-1] if "/" in name else f"{name}.doc"
                try:
                        ...
                        async with session.get(url, headers=headers) as resp:
                            if resp.status == 200:
                                ...
                                with open(filename, "wb") as f:
                                    f.write(content)
                                    print(f"✅ Adda247 file downloaded: {filename}")
                                    copy = await bot.send_document(
                                        chat_id=channel_id,
                                        document=filename,
                                        caption=cc1  # Tumhara existing caption variable
                                    )
                                    count += 1
                                    os.remove(filename)
                            else:
                                print(f"❌ Adda247 Error {resp.status}: {await resp.text()}")
                                await bot.send_message(channel_id, f"⚠️ **Adda247 Download Failed** ⚠️ "
                                                       f"**Name:** `{str(count).zfill(3)} {name1}` "
                                                       f"**Status:** JWT expired - Fresh login karo "
                                                       f"**URL:** `{link0}`")
                                count += 1
                                failed_count += 1
                except Exception as e:
                    print(f"❌ Adda247 Exception: {str(e)}")
                    await bot.send_message(channel_id, f"⚠️ **Adda247 Download Failed** ⚠️ "
                              f"**Name:** `{str(count).zfill(3)} {name1}` "
                              f"**Error:** {str(e)} "
                              f"**URL:** `{link0}`")
            ...
            elif "/masterpl.mpd" in url and "https://videocdn.adda247.com" not in url:
                    for _ in range(3):
                        try:
                            async with aiohttp.ClientSession() as s:
                                async with s.get(f"https://nova-baba-code.vercel.app/iq?url={url}") as r: j = await r.json()
                            break
                        except: await asyncio.sleep(3)
                    mpd = j.get("url"); keys_string = " ".join(f"--key {k}" for k in j.get("keys", []))
            ...
            try:
                cc = (
    f"<b>[📁] 𝑰𝒏𝒅𝒆𝒙 𝑰𝑫:</b> {str(count).zfill(3)} \n\n"
    f"<b>𝑭𝒊𝒍𝒆 𝑵𝒂𝒎𝒆 :</b> {name1} \n\n"
    f"<b>𝑻𝒐𝒑𝒊𝒄 :</b> {topic} \n\n"
    f"<b>𝑩𝒂𝒕𝒄𝒉 𝑵𝒂𝒎𝒆 :</b> {b_name} \n\n"
    f"<b>𝑫𝒐𝒘𝒏𝒍𝒐𝒂𝒅𝒆𝒅 𝑩𝒚:</b> {CR}"
)
                cc1 = (
    f"<b>[📁] 𝑰𝒏𝒅𝒆𝒙 𝑰𝑫:</b> {str(count).zfill(3)} \n\n"
    f"<b>𝑭𝒊𝒍𝒆 𝑵𝒂𝒎𝒆 :</b> {name1} \n\n"
    ...
    f"<b>𝑫𝒐𝒘𝒏𝒍𝒐𝒂𝒅𝒆𝒅 𝑩𝒚:</b> {CR}"
                ...
                if "drive" in url:
                    ...
                    Show = f"<i><b>SUPER SONIC SPEED SE VIDEO DOWNLOAD HO RHA HAI</b></i>\n<blockquote><b>{str(count).zfill(3)}) {name1}</b></blockquote>"
    ...
    channel_id,
    (
        "<b>📬 BATCH Process ᴄᴏᴍᴘʟᴇᴛᴇᴅ</b>\n\n"
        "<blockquote><b>📚 𝐂ⱺᴜʀꜱᴇ ɴᴀᴍᴇ :</b> "
        f"{b_name}</blockquote>\n"
        
        "╭────────────────\n"
        f"├ 🖇️ ᴛᴏᴛᴀʟ Links : <code>{len(links)}</code>\n"
        f"├ ✅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟ : <code>{success_count}</code>\n"
        f"├ ❌ ꜰᴀɪʟᴇᴅ : <code>{failed_count}</code>\n"
        "╰────────────────\n\n"

        "╭──────── 📦 ᴄᴀᴛᴇɢᴏʀʏ ────────\n"
        f"├ 🎞️ Total ᴠɪᴅᴇᴏꜱ : <code>{video_count}</code>\n"
        f"├ 📑 Total ᴘᴅꜰꜱ : <code>{pdf_count}</code>\n"
        f"├ 🖼️ Total ɪᴍᴀɢᴇꜱ : <code>{img_count}</code>\n"
        "╰────────────────────────────\n\n"
        
        "<i>ᴇxᴛʀᴀᴄᴛᴇᴅ ʙʏ NIKHIL BHAI ʙᴏᴛꜱ 🤖</i>"
    )
)

    else:
        await bot.send_message(channel_id, f"<b>-┈━═.•°✅ Completed ✅°•.═━┈-</b>\n<blockquote><b>📚𝐂ⱺᴜʀꜱᴇ Name : {b_name}</b></blockquote>\n<blockquote>🔗 Total URLs: {len(links)} \n┃   ┠🔴 Total Failed URLs: {failed_count}\n┃   ┠🟢 Total Successful URLs: {success_count}\n┃   ┃   ┠🎥 Total Video URLs: {video_count}\n┃   ┃   ┠📄 Total PDF URLs: {pdf_count}\n┃   ┃   ┠📸 Total IMAGE URLs: {img_count}\n┃   ┃   ┠ Successfully Downloaded All Lectures BOSS 😎😎</blockquote>\n")
        await bot.send_message(m.chat.id, f"<blockquote><b>✅ Your Task is completed BOSS 😎😎, Go and check your Set Channel📱</b></blockquote>")
...
async def text_handler(bot: Client, m: Message):
    ...
    try:
            ...
            if "acecwply" in url:
                ...
                url = f"https://itsgolu-cp-api.vercel.app/itsgolu?url={url}@ITSGOLU_OFFICIAL&user_id=7445620075"
                ...
                signed_api = f"https://itsgolu-cp-api.vercel.app/itsgolu?url={url}@ITSGOLU_OFFICIAL&user_id=7445620075"
            ...
            try:
                ...
                elif 'drmcdni' in url or 'drm/wv' in url or "masterpl.mpd" in url:
                    Show = f"**⚡Dᴏᴡɴʟᴏᴀᴅɪɴɢ Sᴛᴀʀᴛᴇᴅ...⏳**\n" \
                           f"🖇️ LNK » {url}\n" \
...
def reset_and_set_commands():
    ...
    # Set new
    commands = [
    {"command": "start", "description": "✅ ᴄʜᴇᴄᴋ Bot jinda hai ki nhi"},
    {"command": "drm", "description": "📄 txt se video kar "},
    {"command": "stop", "description": "⏹ abhi ke process ko rok de"},
    {"command": "reset", "description": "♻️ bot ko dubara on kar"},
    ...
    {"command": "id", "description": "🆔 mera user id kya hai"},
